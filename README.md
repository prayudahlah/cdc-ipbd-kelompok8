...
- **Debezium Connect** — konektor source (PostgreSQL) dan sink (SQL Server)
...
- Port kosong: `2181`, `9092`, `8070`, `5432`, `1434`
...
### 2. Jalankan service source (PostgreSQL + Kafka + Zookeeper + Debezium)
...
### 3. Daftarkan source connector (PostgreSQL → Kafka)
...
### 4. Jalankan service destination (SQL Server)
...
### 5. Daftarkan sink connector (Kafka → SQL Server)
...
```bash
docker exec -i postgres-source psql -U <username> -d kuliah <<< "INSERT INTO mahasiswa VALUES ('12345678', 'John Doe', '2000-01-01');"
```
...
| `.env` | Variabel lingkungan (user, password, dll) dari temlate `.env.example` |
